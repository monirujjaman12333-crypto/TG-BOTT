...
# ─── In-memory cache (refreshed from MongoDB) ───────────────────
numbers_db = {}
used_stats = {}

def load_data():
    global numbers_db, used_stats
    numbers_db = db.get_numbers_db()
    if not numbers_db:
        numbers_db = {k: dict(v) for k, v in default_db.items()}
    for platform, countries in default_db.items():
        numbers_db.setdefault(platform, {})
        for c in countries:
            numbers_db[platform].setdefault(c, [])
    used_stats = db.get_used_stats()
    save_numbers()

def save_numbers():
    db.save_numbers_db(numbers_db)
    db.save_used_stats(used_stats)
...
def load_users_fresh():
    pass  # MongoDB always fresh — no-op
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🔗 Join Channel", url="https://t.me/fb_work_hub")],
...
async def handle_otp_group_message(update, context):
    ...
    matched_uid = None
    matched_key = None
    for stored_num, uid in active_numbers.items():
...
async def admin_withdraws(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = []
    for uid, idx, w, u_data in pending[:10]:
...
async def show_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Remove old active entries for this user
    active_numbers = db.get_active_numbers()
    keys_to_remove = [k for k, v in active_numbers.items() if v == user_id]
    # We'll handle this via user doc update
...
async def admin_country_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    platform = list(numbers_db.keys())[platform_idx]
    buttons = [
        [InlineKeyboardButton(country, callback_data=f"admin_country::{platform_idx}::{c_idx}")]
        for c_idx, country in enumerate(numbers_db[platform].keys())
    ]
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_searching") and user_id == ADMIN_ID:
        ...
        uid = text.strip()
        u = db.get_user(int(uid)) if uid.isdigit() else None
        if not u or u.get("balance") is None:
...
def main():
    ...
    print("🚀 Bot running...")
    app.run_polling(
        drop_pending_updates=True,   # পুরনো pending updates বাদ দাও
